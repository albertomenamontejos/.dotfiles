...
(() => {
      ...
      for (let i = 0; i < properties.length; i++) {
        ...
        if (property.tp === NodeType.PROPERTY) {
          ...
          if (key === value && property.key.tp === NodeType.IDENTIFIER) {
            logger.log('Key = value in object expression, this means injection of variable '
              + property.key.nm + ' into object');
  ...
  window.a2e2916a4fab4416bbef7ec6973f13d6c = (esTree, variables) => {
    const context = {};
  ...
  window.addEventListener('message', event => {
    ...
    if (message.from && message.type === 'request') {
      try {
        const data = window.a2e2916a4fab4416bbef7ec6973f13d6c(...message.data);
