...
var Parser = function(data, moment) {
  ...
  var getBbSeller = function() {
    ...
    var amzRegex = /((ships|dispatched)\s+from(\s|\S)+sold\s+by\s+Amazon)|(sold\s+by:\s+Amazon)|(Expédié\s+et\s+vendu\s+par\s+Amazon)|(Verkauf\s+und\s+Versand\s+durch\s+Amazon)|(Vendido\s+y\s+enviado\s+por\s+Amazon)|(Venduto\s+e\s+spedito\s+da\s+Amazon)|(Amazon.co.jp\s+が販売、発送します。)|(Amazon.co.jp がフラストレーション・フリー・パッケージで販売、発送します)/i
    var amz = amzRegex.test(merchantInfo)

    var fbaRegex = /(fulfilled|ships\s+by|from\s+Amazon)|(sold\s+by:)|(expédié\s+par\s+amazon)|(Versand\s+durch\s+Amazon)|(enviado\s+por\s+Amazon)|(gestionado\s+por\s+Amazon)|(spedito\s+da\s+Amazon)|(が販売し、Amazon.co.jp)/i
    var fba = fbaRegex.test(merchantInfo)

    var merchRegex = /((ships|dispatched)\s+from\s+and\s+sold\s+by)|(Sold\s+by)|(Expédié\s+et\s+vendu\s+par)|(Verkauf\s+und\s+Versand\s+durch)|(Vendido\s+y\s+enviado\s+por)|(Venduto\s+e\s+spedito\s+da)|(^((?!Amazon.co.jp).)*?\s+が販売、発送します。)/i
      ...
      switch (country) {
        case 'jp':
          ...
          if (parsedRank && parsedRank[0] && $.inArray('>', parsedRank) == -1) {
            rank = parsedRank[0].replace(/(位)|(\,)/gi, '').replace(/^\s*\-\s*/gi, '').trim()
          }
          break
        default:
